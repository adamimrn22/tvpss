...
class TableManager
{
    constructor() {
        ...
        this.itemsPerPage = parseInt(document.getElementById('userEntries').value);
        this.currentPage = 1;
    ...
    createNoDataElement() {
        ...
        const noDataContent = `
          <svg  xmlns="http://www.w3.org/2000/svg"  width="80"  height="80"  viewBox="0 0 24 24"  fill="none"   stroke="#adb5bd"   stroke-width="2"  stroke-linecap="round"  stroke-linejoin="round"  class="icon icon-tabler icons-tabler-outline icon-tabler-user-circle"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M12 12m-9 0a9 9 0 1 0 18 0a9 9 0 1 0 -18 0" /><path d="M12 10m-3 0a3 3 0 1 0 6 0a3 3 0 1 0 -6 0" /><path d="M6.168 18.849a4 4 0 0 1 3.832 -2.849h4a4 4 0 0 1 3.834 2.855" /></svg>
            <div style="color: #6c757d; font-size: 1.1rem; margin-bottom: 0.5rem;">
                Tiada pengguna dijumpai
            </div>
            <div style="color: #adb5bd; font-size: 0.9rem;">
               Pastikan anda menaip nama pengguna yang betul atau data pengguna ada
            </div>
        `;
    ...
    initializeControls() {
        ...
        this.entriesSelect = document.getElementById('userEntries');
        this.statusFilter = document.getElementById('userDropdownRole');
        ...
        if (searchTerm === "") {
            ...
            this.filteredRows = this.rows.filter(row => {
                ...
                return searchWords.every(word =>
                    itemName.includes(word) || itemType.includes(word)
                );
            });
        }
    ...
    filterByStatus(status) {
        if (status === 'Semua Pengguna') {
            this.filteredRows = [...this.rows];
        } else {
            this.filteredRows = this.rows.filter(row => {
                // Get the itemStatus and trim it to remove any extra spaces
                const itemStatus = row.children[4]?.textContent.trim() || '';
                const trimmedStatus = status.trim();

                return itemStatus === trimmedStatus;
            });
        }
        this.updateTable();
    }
    ...
    updateTable() {
        ...
        this.filteredRows.slice(startIndex, endIndex).forEach((row, index) => {
            ...
            const userId = row.querySelector('.hover-edit').getAttribute('data-id');
             if (editButton) {
